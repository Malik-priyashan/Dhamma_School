...
const getGridConfig = (width: number, propNumCards?: number): GridConfig => {
    const numCards = propNumCards ?? 6 // Fixed number of cards unless overridden
...
const AnimatedLoadingSkeleton = ({
    ...
    useEffect(() => {
        const config = getGridConfig(windowWidth, propNumCards)
        controls.start(generateSearchPath(config))
    }, [windowWidth, controls, propNumCards])
    ...
    const config = getGridConfig(windowWidth, propNumCards) // Get current grid configuration

    return (
        <motion.div
            className={`${containerClassName} max-w-4xl mx-auto p-6 bg-white rounded-xl shadow-lg`}
                ...
                <div className={gridClassName}>
                    {[...Array(config.numCards)].map((_, i) => (
                        ...
                        >
                            {/* Card placeholders */}
                            <motion.div
                                className={`${thumbClassName} bg-gray-200 rounded-md mb-3 w-full`}
                                animate={{
