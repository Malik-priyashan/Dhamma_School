...
import { usePrefectForm, usePrefectFormUI } from "./hooks/usePrefectForm";
...
export default function PrefectBoardForm() {
  ...
  function validateStepFields(currentStep: number) {
    if (currentStep === 1) {
      const hasName = !!((data.fullNameEn ?? '').toString().trim() || (data.fullNameSi ?? '').toString().trim());
      const hasAddress = !!((data.addressEn ?? '').toString().trim() || (data.addressSi ?? '').toString().trim());
      const hasEntranceDay = !!((data.gradeEntranceDayEn ?? '') || (data.gradeEntranceDaySi ?? ''));
      const hasEntranceNo = !!((data.entranceNoEn ?? '').toString().trim() || (data.entranceNoSi ?? '').toString().trim() || (data.entranceNo ?? '').toString().trim());
      const hasGrade = !!((data.gradeEn ?? '').toString().trim() || (data.gradeSi ?? '').toString().trim() || (data.grade ?? '').toString().trim());
      if (!hasName || !hasAddress || !hasEntranceDay || !hasEntranceNo || !hasGrade) {
        setToast({ show: true, message: t('form.fillRequiredFields') || 'Please fill all required fields', type: 'error' });
    ...
    if (currentStep === 3) {
      ...
      const parentAgree = !!data.parentAgreement;
        ...
        <div className="mb-6 text-center">
          ...
          <form onSubmit={onSubmit}>
            ...
            <div className="flex items-center justify-between">
              ...
              <div>
                {step < 4 ? (
                  <button type="button" onClick={() => { if (validateStepFields(step)) next(); }} className="px-5 py-2 bg-sky-400 hover:bg-sky-500 text-white rounded-full shadow">{t('form.next')}</button>
                ) : (
                  <div>
                    <button
                      disabled={ui.loading}
