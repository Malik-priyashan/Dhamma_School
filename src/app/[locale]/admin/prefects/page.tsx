...
export default function PrefectBoardAdminPage() {
  ...
  const [prefects, setPrefects] = useState<PrefectFormData[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [selectedPrefect, setSelectedPrefect] = useState<PrefectFormData | null>(null);
  ...
  const loadPrefects = async (updatedId?: string, pageNum: number = currentPage) => {
    try {
      setLoading(true);
      const res = await fetchAllPrefects({
        grade: selectedGrade,
      ...
      // If a prefect card is active, update its state with the fresh database values
      const activeId = updatedId || (selectedPrefect ? selectedPrefect.id : undefined);
      if (activeId) {
        const freshRecord = (res.data || []).find((item: any) => item.id === activeId);
        if (freshRecord) {
          setSelectedPrefect(freshRecord);
        }
      }
    } catch (err: any) {
      setError(err.message || "Failed to load prefect applications.");
    } finally {
      setLoading(false);
    }
  };
  ...
  // Load prefects when filters change (reset to page 1)
  useEffect(() => {
    if (isAuthenticated) {
      loadPrefects(undefined, 1);
    }
  }, [debouncedName, selectedGrade, selectedStatus, isAuthenticated]);

  const filteredPrefects = prefects;
  ...
  return (
    <main className="min-h-screen bg-slate-50 flex-1 ml-0 md:ml-64 p-4 md:p-8 pt-20 md:pt-12">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight border-b border-slate-200 pb-4 flex items-center gap-3">
              ...
              Prefect Board Applications
            </h1>
            <p className="text-slate-500 mt-2">
              View and review student applications for the Prefect Board.
            </p>
          </div>
          <div className="flex items-center gap-3 shrink-0">
            <button
              onClick={() => loadPrefects()}
        ...
        {/* Prefects List Table */}
        {!loading && !error && filteredPrefects.length === 0 && (
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-slate-50 border-b border-slate-200">
                      ...
                      <th className="px-6 py-4.5 font-bold text-slate-700 text-xs uppercase tracking-wider">Index No / Entrance No</th>
                      <th className="px-6 py-4.5 font-bold text-slate-700 text-xs uppercase tracking-wider">Grade</th>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {filteredPrefects.map((p) => (
                      <tr
                        key={p.id as string}
                        onClick={() => setSelectedPrefect(p)}
                      ...
                      >
                        ...
                        <td className="px-6 py-4 text-sm text-slate-600">
                          {p.entranceNo || <span className="text-slate-400 font-medium italic">N/A</span>}
                        </td>
                        <td className="px-6 py-4 text-sm text-slate-700 font-medium">
                          {p.grade ? `Grade ${p.grade}` : <span className="text-slate-400 italic">N/A</span>}
                        </td>
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between border border-slate-150 bg-white px-6 py-4.5 rounded-3xl shadow-sm mt-4">
                <div className="flex flex-1 justify-between sm:hidden">
                  <button
                    onClick={() => loadPrefects(undefined, currentPage - 1)}
                  ...
                  <div>
                    <nav className="isolate inline-flex -space-x-px rounded-xl shadow-sm border border-slate-200 overflow-hidden" aria-label="Pagination">
                      ...
                      {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
                        <button
                          key={p}
                          onClick={() => loadPrefects(undefined, p)}
        ...
        {/* Selected Prefect Details Modal Card */}
        {selectedPrefect && (
          <PrefectDetailsCard
            prefect={selectedPrefect}
            onClose={() => setSelectedPrefect(null)}
            onSave={(id) => loadPrefects(id)}
          />
        )}
      </div>
    </main>
