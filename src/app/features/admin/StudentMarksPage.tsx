...
import StudentMarkCard from "../teacher/student-marks/components/StudentMarkCard";
    ...
    async function syncAuth() {
      try {
        ...
        if (isMounted) {
          ...
          const name = user?.name || user?.fullName || (user?.firstName ? `${user.firstName} ${user.lastName || ''}` : "Admin");
          setTeacherName(name.trim() || "Admin");
        }
      } catch {
        if (isMounted) {
    ...
    <main className="min-h-screen bg-slate-50 flex-1 ml-0 md:ml-64 p-4 md:p-8 pt-20 md:pt-12">
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white p-8 rounded-3xl shadow-sm border border-slate-200 animate-in fade-in slide-in-from-top-4 duration-500">
            <div>
                <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight">Student Marks (Admin)</h1>
                <p className="text-slate-500 mt-1 font-medium">Select a grade, set teacher & academic year info, and manage marks with update capability.</p>
            </div>
        ...
        <div className="relative min-h-[500px]">
            {!isInfoSaved ? (
                 <div className="flex flex-col items-center justify-center py-24 bg-white rounded-[2rem] border-2 border-dashed border-slate-200 animate-in fade-in zoom-in-95 duration-500">
                    ...
                    <h3 className="text-2xl font-bold text-slate-800 mb-2">Save Info First</h3>
                    <p className="text-slate-500 max-w-sm text-center font-medium">Please enter and save both the Teacher Name and Academic Year above to start entering marks.</p>
                </div>
            ) : !filterGrade ? (
